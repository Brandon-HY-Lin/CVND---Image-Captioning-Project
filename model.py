...
class DecoderRNN(nn.Module):
    ...
    def sample(self, inputs, states=None, max_len=20, index_end_word=1):
        " accepts pre-processed image tensor (inputs) and returns predicted sentence (list of tensor ids of length max_len) "
        
        search = self.greedy_search
        
        # reset long-term (h) and short-term memories (c)
        ...
        for i in range(max_len):
            ...
            # record word
            sentence_indexes.append(i_word.item())
            
            # stop condition for ending word '<end>'
            if i_word == index_end_word:
                break
